...
$sql = "SELECT A.id, A.ruc,C.razon_social, B.actividad, A.creado, A.vence, A.mes, A.estado,A.fecha_finalizacion,
    ...
    A.ruc=C.ruc and C.razon_social like'%$cnombre%' and B.actividad like'%$cactividad%' and A.estado like'%$ctipo%'
    and (IF(A.vence<curdate() and A.estado='PENDIENTE' ,'VENCIDO',
    IF(A.vence>curdate() and A.fecha_aviso<=curdate() and A.estado='PENDIENTE','PENDIENTE',
    IF(A.vence=curdate() and A.estado='PENDIENTE','HOY',
    IF(A.vence>curdate() and A.fecha_aviso>curdate() and A.estado='PENDIENTE','PROXIMO','SOLUCIONADO'))))) like '%$cestado%' order by vence asc";
...
<table class="table table-striped table-bordered table-hover dataTables-example dataTable" id="DataTables_Table_0" aria-describedby="DataTables_Table_0_info" role="grid">
    ...
    <thead>
        <tr>
            <th>
                <div class="i-checks selectall"><input type="checkbox"  class="check" value=""></div>
            </th>
            <th>ID</th>
            <th>RUC</th>
            <th>RAZON SOCIAL</th>
    ...
    <tbody>
        ...
        ?>
                <tr class="gradeU">
                    <td>
                        <div class="i-checks"><input type="checkbox" id="check" class="check" value=""></div>
                    </td>
